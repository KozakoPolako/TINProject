...
using czatSerwerTIN.Models;

namespace czatSerwerTIN.Hubs
{
    class ConnectionHub : Hub<IConnectionHub>
    {
        private readonly List<UserModel> _Users;
        private readonly List<UserCall> _UserCalls;
        private readonly List<models> _CallOffers;

        public ConnectionHub(List<UserModel> users, List<UserCall> userCalls, List<models> callOffers)
        {
            _Users = users;
        ...
        public async Task CallUser(UserModel targetConnectionId)
        {
        ...
        public async Task AnswerCall(bool acceptCall, UserModel targetConnectionId)
        {
            var callingUser = _Users.SingleOrDefault(u => u.ConnectionId == Context.ConnectionId);
            var targetUser = _Users.SingleOrDefault(u => u.ConnectionId == targetConnectionId.ConnectionId);
            ...
            {
                Users = new List<UserModel> { callingUser, targetUser }
            });
    ...
    {
        Task UpdateUserList(List<UserModel> userList);
        Task CallAccepted(UserModel acceptingUser);
        Task CallDeclined(UserModel decliningUser, string reason);
        Task IncomingCall(UserModel callingUser);
        Task ReceiveSignal(UserModel signalingUser, string signal);
        Task CallEnded(UserModel signalingUser, string signal);
    }
}
